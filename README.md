...
New Claude stages default to `claude-opus-5-5` with explicit `high` effort. `AGENT_HARNESS_CLAUDE_MODEL` remains an explicit override under the existing Opus 5-or-newer policy. Updating the plugin does not rewrite stored run or campaign execution settings, change the current Codex conversation's model, or restart an active Claude stage. Model availability and subscription access must be checked on each host; `check_runtime` does not invoke inference or prove access to the selected model.

Prepared tasks persist their native executor settings, acceptance criteria, constraints, required checks, versioned contract references, correction budget, and critic-retry budget. The default executor is `gpt-6-sol` with `high` reasoning effort and no default escalation model; an epic plan may explicitly record `gpt-6-astra` for planning or justified escalation. The Codex lead passes the executor settings to the native task. The MCP server never launches or attests Sol itself. Claude remains the fresh read-only critic.

Campaigns may run several dependency-free tasks in parallel waves. Combining several slices requires a checked integration commit; a single-slice wave passes its verified source SHA directly, without duplicate review of an identical tree. Legacy campaign contracts remain unchanged. Local execution dependencies are separate from `parallel`, `stacked`, and `after_merge` publication order. Code corrections receive current checks and scoped independent review; a [narrow nonsemantic P3 closeout](plugins/codex-agent-harness/skills/workflow/references/review-followups.md) reruns checks while retaining original review provenance. A bounded explicit retry is available only for classified transient critic timeouts or process failures. A confirmed Anthropic usage limit still uses the campaign cooldown, later recovery probe, and visibly marked `codex_fallback`; it is never treated as a generic provider switch.
