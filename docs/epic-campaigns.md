...
The campaign freezes its source, goal, restrictions, risk, approved specification fingerprint, runtime version, and ordered tasks. A prepared task may contain:
...
New tasks default to `gpt-6-sol` with `high` reasoning effort, two correction passes, and one transient critic retry. A legacy missing correction value remains one pass; a legacy missing critic-retry value remains zero. The runtime default for `execution.escalation_model` is null; an epic plan may explicitly set `gpt-6-astra`. The lead passes the stored native model and effort explicitly when spawning the executor. These values are planning metadata: MCP stores them but does not launch or attest the native model.

A campaign-linked run must use the task goal. It inherits completion criteria, constraints, non-goals, forbidden actions, required checks, pinned references, execution settings, review budget, and recovery budgets. The caller may add criteria, constraints, non-goals, forbidden actions, checks, and references, but cannot override a same-name check or any frozen setting. For native campaigns, `spec_context` exposes absolute paths to the immutable common and task-specific snapshots; an executor reads only those two unless its task explicitly references a dependency's spec. Legacy OpenSpec still contributes a pinned `openspec:<change-id>` reference using its semantic SHA-256.

## Readiness and specifications

An approved specification is required for a large, ambiguous, high-risk, multi-task, or behaviorally complex change. Investigate the actual technical landscape before decomposing it. Native Markdown is the default: `.agent-harness/specs/<change-id>/spec.md` holds epic-wide architecture, contracts and invariants; `tasks/<task-id>.md` holds each prepared task's behavior, technical scope, verification scenarios, dependencies and PR boundary. Every campaign task needs an exact-ID file. A standalone `create_run` may optionally use just `spec.md`. `create_campaign` and standalone `create_run` accept `spec: {kind: "harness", change_id: "<change-id>", readiness: "ready" | "analysis_required"}`. Neither the OpenSpec CLI nor OpenSpec headings, YAML, or MUST boilerplate apply. Readiness requires affected components, contract revisions and availability, states and errors, recovery, security, compatibility, rollout, publication, and rollback boundaries. Every requirement maps to at least one task; every task maps to a verification scenario. Ordinary technical choices can remain for the lead. Product, scope, or task-boundary uncertainty becomes an `analysis` task before implementation.

Native drafts are ignored locally through `/.agent-harness/specs/` in `.git/info/exclude`. Approval covers the common and all task specs as one packet. Creation freezes a private mode-`0600` snapshot in Git metadata; later draft edits neither block an active campaign nor change its approved contract. A material plan change needs a new approved scoped contract, not a silent rewrite or automatic revision migration. OpenSpec remains an explicit and legacy mode: local `/openspec` is ignored, while `storage: repository` preserves versioned specs and its finalization run. Its checkbox normalization and content-change gate apply only to OpenSpec. Existing campaigns retain their chosen format. The 300–700 production-line budget remains advisory and is reported separately from tests, documentation, configuration, generated files, and binaries.
