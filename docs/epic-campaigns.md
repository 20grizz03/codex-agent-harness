...
For a live large idea, high-risk change, or multi-task epic, `epic-workflow` requires an installed and initialized OpenSpec checkout to keep the approved proposal, behavioral deltas, design decisions, and task list in Git. One already-ready Jira task does not need a separate change. OpenSpec does not replace campaign state: its checkboxes are the plan, while terminal task state and evidence remain in Agent Harness.

The first OpenSpec task checks actual dependency capabilities, including registry reachability, without requiring VPN as the mechanism. After every checked task and the combined integration run are complete, a separate `role: finalizer` v1 run named `openspec-finalize` validates and archives the change, then runs its own checks and review. OpenSpec is never initialized automatically and is not used for blind replay.
