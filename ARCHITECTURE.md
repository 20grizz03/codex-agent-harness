...
Codex owns the user conversation, native executor creation, repository mutations, project-command execution, finding triage, and the completion decision. A prepared campaign task records `execution.native_model`, `execution.reasoning_effort`, and an optional `execution.escalation_model`; the lead passes the first two values explicitly to the native executor. Runtime defaults are `gpt-6-sol`, `high`, and a null escalation model; an epic plan may explicitly name `gpt-6-astra` for planning or recorded escalation. The MCP server owns immutable contracts, durable state, check planning, evidence validation, and Claude process lifecycle, but never launches or attests Sol. Claude never commits, pushes, publishes, deploys, or mutates external systems.

The MCP server does not execute project checks. It returns argv arrays derived from the immutable contract and `.codex/agent-harness.json`; Codex executes them with its existing sandbox and egress controls, then records bounded outcomes. Native specifications use internal schema version `1` and a private mode-`0600` snapshot: one common `spec.md` plus one exact-ID task file per campaign task, or only `spec.md` for a standalone run. `spec_context` returns absolute snapshot paths. Working Markdown drafts are not execution inputs after creation; changing approved behavior requires a new scoped contract rather than mutation of the snapshot. Existing OpenSpec contracts retain their legacy format and semantics.
