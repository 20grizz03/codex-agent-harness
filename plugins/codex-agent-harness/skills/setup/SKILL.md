...
1. Resolve `codex`, `git`, `python3`, `claude`, `node`, and `openspec` from `PATH` and report their paths and public versions. Confirm that the prepared task's native model and any recorded escalation model are available on the target Codex host; this is a capability check, not an MCP attestation. OpenSpec is optional; when present, require Node.js 20.19.0 or newer and invoke diagnostics through `env OPENSPEC_TELEMETRY=0`.
...
5. If OpenSpec is present, use `env OPENSPEC_TELEMETRY=0 openspec schema which agent-harness` to report whether the bundled personal schema has been installed. For a prepared campaign, also confirm that every pinned `{ref, revision}` contract can be resolved or is present in its immutable snapshot. Do not reproduce contract contents during setup.
