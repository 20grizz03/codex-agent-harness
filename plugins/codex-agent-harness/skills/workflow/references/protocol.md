...
Before `create_run`, classify the change by behavior and risk. An approved specification is required for the complex boundaries below. For example, one Jira issue that runs Google, Facebook, and Yandex independently with retries and partial success still needs an approved one-task campaign. Native Markdown is the default; existing `kind: openspec` campaigns and an explicit user choice retain OpenSpec.
...
If a completed candidate has dependent branches, use the [correction chain](correction-chain.md). Mechanical adaptation builds each affected PR and runs affected tests on terminal candidates with their ancestors' accumulated scope, without an automatic new critic; semantic changes create a scoped new run linked to the follow-up. Do not reopen an already completed run or treat the old review as current evidence for a changed result.
...
Before editing on the standard path, call `create_run` with the goal, observable `done_when`, non-goals, constraints, forbidden actions, risk, required checks, pinned `contract_refs`, `review_budget`, `max_correction_passes`, and `max_critic_retries`. Each contract reference has the form `{ref, revision}`. An independent run may optionally pass `spec: {kind: "harness", change_id: "<change-id>", readiness: "ready"}` and needs only `.agent-harness/specs/<change-id>/spec.md`, not an epic or task files. Its approved snapshot is returned as an absolute path in `get_run.spec_context`. New contracts default to two correction passes and at most one transient critic retry. A missing legacy correction value remains one; a missing legacy retry value remains zero.
...
For a correction after completion, pin the old and new parent SHAs and each descendant's own commit range before changing a branch. The [correction chain](correction-chain.md) defines who owns the transfer, how to preserve active writers, and which evidence a mechanical adaptation needs.
...
Before another cycle of an unfinished run, read [review follow-ups](review-followups.md). For code corrections, record accepted findings unresolved, fix them as one bounded pass, call `plan_checks(begin_correction: true)`, and rerun every required check. The new independent review focuses on previous findings, the delta and affected relationships when a verified base is available. Keep history; several edits before the next check/review boundary count as one correction cycle. Stop when immutable `max_correction_passes` is exhausted. A validated `nonsemantic_closeout` does not call the model or consume a correction cycle, but requires current checks and resolved P3 findings. After completion, use the [correction chain](correction-chain.md) for a candidate with dependents, or choose a small publication follow-up or new scoped run for a standalone candidate.
...
Load `delivery-writing`, call `get_run` for every candidate and `get_campaign` when present, then verify each full Git result against its own frozen base. Standard candidates must have complete state, matching paths and fingerprint, green current checks, and a resolved current review. An explicitly recorded small publication follow-up instead preserves this evidence for its baseline and labels its delta unverified under the publication reference. A mechanically adapted chain uses each candidate's exact follow-up reference and own current build, plus affected tests on terminal candidates covering their ancestors; its old runs do not attest the changed trees. A semantic correction requires its new complete scoped run. Unexplained mismatches stop the package. Perform the local manual scenario unless the small follow-up route applies, refresh external state, and show one package with the implementation summary, categorized size, verification limitations, commit, PR text, Jira testing recommendations, and exact proposed actions.
